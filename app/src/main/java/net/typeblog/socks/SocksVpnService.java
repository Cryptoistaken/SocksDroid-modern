...
public class SocksVpnService extends VpnService {
	...
	private void start(int fd, String server, int port, String user, String passwd, String dns, int dnsPort, boolean ipv6, String udpgw) {
		...
		command.add("--dnsgw");
		command.add("26.26.26.1:8091");
		
		if (udpgw != null && !udpgw.isEmpty()) {
			command.add("--udpgw-remote-server-addr");
			command.add(udpgw);
		}
		
		if (DEBUG) {
			Log.d(TAG, java.util.Arrays.toString(command.toArray()));
		}
		
		if (Utility.exec(command.toArray(new String[0])) != 0) {
			stopMe();
			return;
		}
